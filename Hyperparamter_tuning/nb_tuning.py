...
def hyperparameter_tuning_nb(X_train, y_train, X_test, y_test, X_columns):
    best_hyperparameters = {}
    best_fair_hyperparameters = {}
    best_accuracy = 0
    best_fair_accuracy = 0

    # Hyperparameter grids for GaussianNB
    var_smoothing_options = np.logspace(-9, -1, 10)

    # Iterate over all values of var_smoothing
    for var_smoothing in var_smoothing_options:
        nb = GaussianNB(var_smoothing=var_smoothing)
        accuracy = np.mean(cross_val_score(nb, X_train, y_train, cv=5))
    ...
    with open("Hyperparamter_tuning/nb_hyperparameters.txt", "w") as file:
        file.write(f"Best Hyperparameters for Naive Bayes: {best_hyperparameters}\n")
...
if __name__ == "__main__":
    ...
    best_hyperparameters, best_accuracy, best_fair_accuracy = hyperparameter_tuning_nb(X_train, y_train, X_test, y_test, X.columns)

    # Running the best Naive Bayes model with tuned hyperparameters
    nb_best = GaussianNB(**best_hyperparameters)
    nb_best.fit(X_train, y_train)
    y_pred_best = nb_best.predict(X_test)
    final_accuracy = np.mean(cross_val_score(nb_best, X_train, y_train, cv=5))

    with open("Hyperparamter_tuning/nb_hyperparameters.txt", "a") as file:
